...
test('basic', function(t) {
  ...
  ac.get(__filename, load, afterFirst);
  ...
  ac.get(__filename, load, afterSecond);
  ...
  function next() {
    ...
    t.equal(called, 2);
    ac.get(__filename, load, function(er, st) {
      if (er) throw er;
  ...
  function next2() {
    ...
    ac.get(__dirname, load, function(er, st) {
      if (er) throw er;
      t.type(ac.peek(__filename), 'undefined');
      t.equal(ac.itemCount, 1);
      ac.get(__filename, load, function(er, st) {
...
test('allow stale', function(t) {
  ...
  t.equal(ac.itemCount, 0);
  ac.get('foo', load, function(er, val) {
    t.equal(ac.itemCount, 1);
    t.equal(val, 0);
    var start = Date.now();
    setTimeout(function() {
...
test('return stale while updating', function(t) {
  ...
  function step() {
    ac.get('someKey', load, function(err, item) {
